...
class PCDAlign(nn.Module):
    ...
    def forward(self, fea1, fea2):
        ...
        # L2_offset = self.lrelu(self.L2_offset_conv2_1(L2_offset))
        ...
        L2_offset = self.lrelu(
            self.L2_offset_conv2_1(torch.cat([L2_offset, (L3_offset * 2)[:,:,0:16,0:45]], dim=1))
        )
        ...
        L2_fea = self.lrelu(self.L2_fea_conv_1(torch.cat([L2_fea, (L3_offset)[:,:,0:16,0:45]], dim=1)))
        # L1
        ...
        L1_offset = self.lrelu(
            self.L1_offset_conv2_1(torch.cat([L1_offset, (L2_offset * 2)[:,:,0:31,0:90]], dim=1))
        )
        ...
        L1_fea = self.L1_fea_conv_1(torch.cat([L1_fea, L2_fea[:,:,0:31,0:90]], dim=1))
...
class Easy_PCD(nn.Module):
    ...
    def forward(self, f1, f2):
        ...
        L2_fea = self.lrelu(self.fea_L2_conv2(L2_fea))    # torch.Size([32, 64, 17, 45])
        # L3
        L3_fea = self.lrelu(self.fea_L3_conv1(L2_fea))
        L3_fea = self.lrelu(self.fea_L3_conv2(L3_fea))  # torch.Size([32, 64, 9, 23])
        
        # import pdb
        # pdb.set_trace()
        L1_fea = L1_fea.view(B, N, -1, H, W)
        L2_fea = L2_fea.view(B, N, -1, 16, 45)
        L3_fea = L3_fea.view(B, N, -1, 8, 23)
        # L3_fea = L3_fea.view(B, N, -1, H // 4, W // 4)
        # import pdb
        # pdb.set_trace()
        # L2_fea = L2_fea.view(B, N, -1, 17, 45)
        # L3_fea = L3_fea.view(B, N, -1, 9, 23)
