...
from Spk2ImgNet.align_arch_mv01 import *
...
class SpikeNet(nn.Module):
    def __init__(self, in_channels, features, out_channels, win_r, win_step):
        ...
        input_flatten_size = 31 * 90  # 输入空间维度展平    
        self.fc = nn.Linear(input_flatten_size, 90 * 90)
        self.relu = nn.ReLU()

    def forward(self, x):
        # import pdb
        # pdb.set_trace()
        x = x.reshape(x.size(0), 1, x.size(1), x.size(2)).repeat(1, 54, 1, 1)
        ...
        out = self.relu(self.fc(out.view(out.size(0), -1)))
        out = out.view(out.size(0), 1, 90, 90)
